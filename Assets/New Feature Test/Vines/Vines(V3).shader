Shader "CityBuster/EdgeVines"
{
    // ----------------------------------------------------------------------
    // Border-telegraph overlay: thorny vines grow INWARD from the four
    // screen edges. The "soil" (mottled red/black cellular noise) is the
    // root material sitting flush against the border; tendrils taper to
    // glowing points as they reach toward the interior.
    //
    // No flat vignette color layer — the soil noise pattern IS the base
    // fill. Growth is driven externally via _GrowthAmount (0 = hidden,
    // 1 = fully grown to _TendrilLength), intended to be wired to
    ...
    Properties
    {
        ...
        _TendrilLength   ("Max Growth Length (UV units)",     Range(0.01, 0.6)) = 0.35
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings IN) : SV_Target
            {
                ...
                float growthMax = saturate(_GrowthAmount) * _TendrilLength;
                ...
                float tendrilLen = growthMax * (0.35 + 0.85 * rLen) * (0.35 + 0.65 * growPulse);
                ...
                // Fixed vs. original: edge0 < edge1 now (was inverted,
                // which is undefined per the HLSL smoothstep spec even
                // though it happened to render on desktop). Same result,
                // spec-legal on Android/IL2CPP too.
                ...
                // ---- soil / root mass: mottled cellular pattern, no
                // flat color layer. Covers the border out to roughly
                // growthMax, with a jagged inner edge. ----------------------
                float soilJag = FBM(float2(lane * 6.0 + edgeId * 3.0, t * 0.05)) - 0.5;
                float soilReach = max(growthMax * (1.0 + soilJag * 0.35 * _EdgeJaggedness), 0.0001);
                ...
                float soilAlpha = soilCoverage * lerp(_SoilColorA.a, _SoilColorB.a, veins);
