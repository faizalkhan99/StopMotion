    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half ValueNoise(half2 p)
            {
                ...
                half a = Hash21(i);
                half b = Hash21(i + half2(1.0, 0.0));
                half c = Hash21(i + half2(0.0, 1.0));
                half d = Hash21(i + half2(1.0, 1.0));
            ...
            half Voronoi(half2 p)
            {
                ...
                for (int y = -1; y <= 1; y++)
                {
                    ...
                    for (int x = -1; x <= 1; x++)
                    {
                        ...
                        half2 cellSeed = i + neighbor;
                        half2 pointPos = neighbor + half2(Hash21(cellSeed), Hash21(cellSeed + 19.19)) - f;
            ...
            half4 Frag(Varyings IN) : SV_Target
            {
                ...
                // Wrapped so values fed into half-precision noise calls
                // stay small and precise even after long play sessions.
                // 1000s period, sin()/noise are periodic so the wrap is
                // visually seamless.
                float t = fmod(_Time.y * _AnimationSpeed, 1000.0);
                ...
                float rimNoise = FBM2(half2(angleN * 6.0, t * 0.06)) - 0.5;
                ...
                float growPulse = 0.5 + 0.5 * FBM2(half2(sectorId * 0.37, t * (0.15 + 0.25 * rSpeed)));
                ...
                float n = FBM(half2(dist * _NoiseScale, warpUV1)) - 0.5;
                ...
                // ---- glowing tip ------------------------------------------
                // Voronoi (9 hash-pair samples) only ever affects the
                // final color where smoothstep(0.55,1.0,tNorm) is nonzero.
                // Skip it entirely everywhere else in the band.
                float voronoiBreak = 0.5;
                if (tNorm > 0.5)
                {
                    voronoiBreak = Voronoi(half2(sectorId * 1.7, dist * _NoiseScale * 0.5 + t * 0.1));
                }
